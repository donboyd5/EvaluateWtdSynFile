...
recipe <- recipe %>%
  ...
  ungroup %>%
  mutate(scale=ifelse(target!=0, target * 1000, 1),
         priority.weight=case_when(fn %in% c("n.sum", "val.sum") ~ 1,
                                   var %in% c("wt", "c00100", "e00200") ~ 1,
                                   TRUE ~ 1)) %>%
  mutate(multiplier=priority.weight * scale) %>%
  mutate(obj.element=paste0(var, "_", fn)) %>%
  select(obj.element, var, fn, scale, priority.weight, multiplier, target)
recipe
...
coeffs <- expand.grid(wtnum=1:nrow(inputs$synsub), obj.element=inputs$recipe$obj.element, stringsAsFactors = FALSE) %>%
  left_join(inputs$recipe %>% select(obj.element, var, fn, multiplier, target)) %>%
  left_join(synlong) %>%
...
# x <- xlb
# x <- xub
# d1 <- eval_grad_f_full(x, inputs)
# d2 <- eval_grad_f_full_2(x, inputs2)
# 
# # calculate the gradient for a particular set of weights
# wdf <- tibble(w=x, wtnum=1:length(w))
# 
# grad <- coeffs %>% 
#   left_join(wdf) %>%
#   group_by(obj.element) %>%
#   mutate(grad=2 * priority.weight*coeff*(sum(w*coeff) - mean(target))) %>%
#   group_by(wtnum) %>%
#   summarise(grad=sum(grad))
# ht(grad)
# 
# 
# 
# tibble(d1, d2) %>% mutate(diff=d1 - d2, pdiff=diff / d2 * 100) %>% ht(10)
# 
# d1[1]
# d2[1]
...
eval_f_full <- function(w, inputs) {
  ...
  temp <- inputs$recipe %>%
    ...
    mutate(target.calc=do.call(fn, list(inputs$synsub, var, w))) %>% 
    ungroup %>%
    mutate(obj=multiplier * (target.calc - target)^2)
...
eval_grad_f_full <- function(w, inputs){
  ...
  #   wrt w1:          2*p*a1*(w1*a1 + w2*a2 + w3*a3 - s)
  #   wrt w2:          2*a2*p*(w1*a1 + w2*a2 + w3*a3 - s)
  #   wrt w3:          2*a3*p*(w1*a1 + w2*a2 + w3*a3 - s)
  # If we have multiple target elements, each vector element will have a more complex sum

  wdf <- tibble(wt.iter=w, wtnum=1:length(wt.iter))
  
  grad <- inputs$coeffs %>% 
    left_join(wdf, by="wtnum") %>%
    group_by(obj.element) %>%
    mutate(grad=2 * multiplier*coeff*(sum(wt.iter*coeff) - first(target))) %>%
    group_by(wtnum) %>%
    summarise(grad=sum(grad))
...
eval_f_full_scaled <- function(w, inputs) {
  ...
  temp <- inputs$recipe %>%
    ...
    mutate(target.calc=do.call(fn, list(inputs$synsub, var, w))) %>% 
    ungroup %>%
    mutate(obj=multiplier * (target.calc / scale - target / scale)^2)
...
eval_grad_f_full_scaled <- function(w, inputs){
  ...
  grad <- inputs$coeffs %>% 
    left_join(wdf, by="wtnum") %>%
    group_by(obj.element) %>%
    mutate(grad=2 * multiplier*coeff*(sum(wt.iter*coeff) - first(target)) / {first(target)^2}) %>%
    group_by(wtnum) %>%
    summarise(grad=sum(grad))
